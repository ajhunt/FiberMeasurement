...
function [intensity, directionVector, angle, thetaRight, thetaLeft, indRight, indLeft, braidAngle, rightBand, leftBand, averageAngleDistribution, sortedCirclePoints] = braidanglescanBresenham(edgeBraidFreqSpectrum)
...
% Scanning for the right braid angle
for i = 1:count/2
...
angleBand = .85; % the percentage that the peak intensity must decrease before the directions are not included in the angular span

% Angular distribution of the right braid angle
for j = indRight:size(intensity, 2)/2
    if intensity(j)<= intensityRight*angleBand
        rightBand(1) = -1*angle(j); 
        break
    end
end
...
for j = indRight:-1:1
   if intensity(j)<= intensityRight*angleBand
       rightBand(2) = -1*angle(j); 
       break
   end
end
if numel(rightBand) == 1
    rightAngleDistribution = 0;
else
rightAngleDistribution = abs(rightBand(1)-rightBand(2));
end

%Angular distribution of the left braid angle
for j = indLeft:size(intensity, 2)
    if intensity(j)<= intensityLeft*angleBand
        leftBand(1) = angle(j); 
        break
    end
end

for j = indLeft:-1:size(intensity, 2)/2
   if intensity(j)<= intensityLeft*angleBand
       leftBand(2) = angle(j); 
       break
   end
...
if numel(leftBand) == 1
    leftAngleDistribution = 0;
else
leftAngleDistribution = abs(leftBand(1) - leftBand(2));
end
%Finding average distribution size
averageAngleDistribution = (rightAngleDistribution+leftAngleDistribution)/4;
